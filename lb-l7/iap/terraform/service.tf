...
resource "google_service_account" "run" {
  project      = google_project.main.project_id
  account_id   = "${var.project_id_prefix}-run"
  display_name = "${var.project_id_prefix}-run"
  description  = "Service Account for ${var.project_id_prefix}-run"
}

resource "google_cloud_run_v2_service" "main" {
  project  = google_project.main.project_id
  location = var.region
  name     = "${var.project_id_prefix}-run"
